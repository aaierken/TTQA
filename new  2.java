...
Finding WebElement  by using various locators
...
Inserting string in Text Field
...
forward() – use to navigate one step forward in browser historydriver.navigate().forward();
refresh() – This will refresh you current open urldriver.navigate().refresh();
Deleting all Cookies before doing any kind of action  driver.manage().deleteAllCookies();
...
builder.keyDown(Keys.CONTROL).click(someElement).click(someOtherElement).keyUp(Keys.CONTROL).build().perform();When we press multiple keys or action together then we need to bind all in a single command by using build() method and perform() method intend us to perform the action.
